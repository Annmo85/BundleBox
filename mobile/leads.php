<?php
	...
	if (isset($request['id'])) {
		...
		//Глянем в кеш
		if (file_exists(__DIR__.'/leadscache/leads_'.$request['id'].'.json')) {
			$jsonString = file_get_contents(__DIR__.'/leadscache/leads_'.$request['id'].'.json');
	...
	//UF_CRM_1668785031029
		...
		foreach ($result['result'] as $lead) {
				...
				$response['deals'][] = [
					...
					'SUM'=>round($lead['OPPORTUNITY'],0) . " руб",
					'd'=>$lead
				];
		...
		$orders_count = 0;
		$response['deals_for_payement'] = [];
		$response['all_delivery_summ'] = 0;
		$response['all_delivery_qty'] = 0;
		
		$response['all_payments_summ'] = 0;
		$response['all_payments_qty'] = 0;
		
		foreach($response['deals'] as $d) {
			$orders_count++;
			if ($d['STAGE']=='Оплатите заказ' || $d['STAGE']=='Отправлен счёт' || $d['STAGE']=='Оплатите доставку' || $d['STAGE']=='Получите заказ') {
				$response['deals_for_payement'][]=$d;
				$badge_count++;
			}
				
			if ($d['STAGE']=='Оплатите доставку') {
				$response['all_delivery_summ']+=round($lead['OPPORTUNITY'],0);
				$response['all_delivery_qty']++;
				
			}						
			if ($d['STAGE']=='Оплатите заказ') {
				$response['all_payments_summ']+=round($lead['OPPORTUNITY'],0);
				$response['all_payments_qty']++;
				
			}
		}
		$response["badge_count"] = $badge_count;
		$response["orders_count"] = $orders_count;
		$response['all_delivery_summ_p'] = $response['all_delivery_summ'] . " руб";
		$response['all_payments_summ_p'] = $response['all_payments_summ'] . " руб";
		...
		$fp = fopen(__DIR__.'/leadscache/leads_'.$request['id'].'.json', 'w');
