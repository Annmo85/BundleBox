...
/**
	Переделан для обратной совместимости с смс.ру на смсаэро
*/
	include_once('smsaero.class.php');
...
class SMSRU {
  ...
  private $domain = 'gate.smsaero.ru/send/';
...
/*
			$post->to = $phone;
			$post->text = 'Ваш пароль: '.$code; 
*/
  

	$smsaero_api = new SmsaeroApiV2('bundlebox38@gmail.com', 'OzRcgBgYE7jXYv88fgCc01pAONZ'); // api_key из личного кабинета
	$result  = $smsaero_api->send([$post->to],$post->text);
	
	$ret = new \stdClass();
	$ret->status = "";
	if (isset($result['success']) && $result['success']) $ret->status = "OK";
	//print_r($result);
